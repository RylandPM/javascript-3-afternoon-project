...
function employeeUpdater() {
  let updatedEmployees = employees.filter(function(employee, i, employees) {
...
function removeDuplicates() {
  ...
  for (let i = 0; i < workplaceAccidents.length; i++) {
    let dupeFind = false;
    for (let x = 0; x < fixedArr.length; x++) {
      if (workplaceAccidents[i] === fixedArr[x]) {
        dupeFind = true;
      }
    }
    if (dupeFind === false) {
      fixedArr.push(workplaceAccidents[i]);
    }
  }
  workplaceAccidents = fixedArr;
